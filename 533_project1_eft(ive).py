...
"""
It is evident that the p-value for Selected_Month stands at 0.127, significantly exceeding the generally accepted threshold of 0.05. Furthermore, the R-Squared value is reported to be just 0.008.
These figures indicate that the model is ineffective in providing a substantial explanation for the variance in returns observed in November.
...
"""
The p-value for Selected_Month is now 0.009, falling below the standard threshold of 0.05. This suggests that the returns for September are statistically distinct from other months.
However, the modest R-squared value of 0.02 indicates that Selected_Month accounts for only a minimal portion of the variance in the dependent variable, Returns. This implies that the model is still not an ideal fit.
The confidence intervals, ranging from -0.045 to -0.006, reinforce the notion of a negative impact on September returns.
In summary, this model demonstrates a better capacity to account for the negative returns observed in September when compared to its predecessor.
...
"""
The revised model presents an impressive adjusted R-squared value of 0.97 and an F-statistic of 1269, both indicating a strong and reliable model.
The Intercept value at -0.0390 suggests a lower than expected performance for the ETF.
Diagnostic tests like Omnibus, Durbin-Watson, and Jarque-Bera yield typical results, suggesting a predominantly normal distribution of the data.
The high value of MktRF aligns with expectations, while the positive coefficients for HML and CMA suggest these factors significantly influence the ETF's performance.
Based on this analysis, it appears that the ETF is negatively influenced by factors such as SMB, RMW, and MOM, and positively affected by HML and CMA factors.
...
def test_industryrets(etf_ticker, start):
    ...
    model2019 = smf.ols("Returns ~ Other * post2019", data=merged_data).fit()
    print(model2019.summary())

test_industryrets('IVE', '2020-01-01')

"""
The model yields a strong R-Squared value and a high F-statistic, indicating a solid fit.
Analysis of the coefficients reveals that the most significant exposure is towards the 'Other' category, with 'NoDur' and 'Manuf' following closely.
In the latter part of the analysis, we assess the change in the ETF's exposure to the 'Other' industry before and after the year 2019.
The coefficient for 'Other:post2019' shows an increase of 0.4153, suggesting a heightened exposure of the ETF to the 'Other' industry post-2019.
"""